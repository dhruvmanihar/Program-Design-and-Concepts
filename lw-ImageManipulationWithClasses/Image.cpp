...
Image::Image(const Image& old) : image(nullptr), width(0), height(0)  {
  allocateImage(old.width,old.height);
  for(unsigned int i = 0;i<old.width;i++){
    for(unsigned int j=0;j<old.height;j++){
      image[i][j] = old[i][j];
    }
  }
...
Image& Image::operator=(const Image& objToCopy){
  if (this != &objToCopy) {           // 1. Don't self-assign
      clear();                  // 2. Delete old dataObject
      allocateImage(objToCopy.width,objToCopy.height);               // 3. Allocate new dataObject
      for(unsigned int i = 0;i<objToCopy.width;i++){
        for(unsigned int j=0;j<objToCopy.height;j++){
          image[i][j] = objToCopy[i][j];
    }
  }// 4. Copy dataObject
}
